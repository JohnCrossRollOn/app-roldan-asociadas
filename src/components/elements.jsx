export default {
  jumbotron: {
    title: 'Consultenos',
    subtitle: 'Somos el estudio de abogacia por excelencia.',
    image: 'https://placeimg.com/1920/1080/arch',
  },
  ourselves: {
    title: 'Somos un equipo de abogados que brinda un asesoramiento integral',
    subtitle:
      'El estudio jurídico Roldan & Asociadas está integrado por profesionales de excelencia, orientados a responder eficientemente todas las necesidades jurídicas de nuestros clientes, acompañándolos en el proceso de manera personalizada directa, generando comodidad y confianza.',
  },
  services: [
    [
      'Divorcios, Familia y Sucesiones',
      'Profesionales calificados para asesorar y resolver todos los asuntos de familia. Alimentos y Cese de alimentos, Responsabilidad Parental, Cuidado personal, Regimen de Comunicación.',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737202/app-roldan-asociadas/R_qnxtud.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
      [
        'Jubilaciones y Pensiones',
        'Consultoria Previsional, Jubilación Planificada, Pensión directa y derivada por fallecimiento del trabajador en actividad o jubilado, Jubilación para Autónomos.',
        'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737220/app-roldan-asociadas/pensiones_sj0mji.jpg',
        [
          ['Pregunta Frecuente', 'Respuesta Frecuente'],
          ['Pregunta Frecuente', 'Respuesta Frecuente'],
          ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ],
      ],
    ],
    [
      'Derecho Laboral',
      'Despidos, Reclamos laborales, Trabajo en negro, Suspenciones, Personal de Casas Particulares',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737233/app-roldan-asociadas/1675w-x-1275h_0005_Layer-17_gkajix.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
    [
      'Accidentes Laborales ART',
      'Abogados especialistas en Accidentes de Trabajo y Enfermedades Profesionales.',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737241/app-roldan-asociadas/R.c366c6d07bf4c4fdd58526a8e9cf7613_kehhmq.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
    [
      'Accidentes de Tránsito',
      'Abogados con sólida experiencia en reclamos y daños materiales contra las compañías de seguros. Auto, Moto, bicicleta, Transportados y Peatón.',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737252/app-roldan-asociadas/N6LXFGDWQILBOHU5PARNUDGDHQ_xkmah4.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
    [
      'Infracciones de Tránsito',
      'Somos abogados especialistas en Derecho de Faltas de Tránsito. Brindamos un Servicio Legal para que no pagues de más, resolvemos tus multas de tránsito de forma on-line.',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737266/app-roldan-asociadas/multas-1-3ebm8jv2c7ng-770x470_cjxxh1.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
    [
      'Escrituración y Usucapión',
      'Abogados especialistas en Escrituracion y Usucapion',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737275/app-roldan-asociadas/usucapion-768x576_o3e18l.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
    [
      'Gestoría del Automotor',
      'Nuestros profesionales realizan diferentes trámites en los Registros de la Propiedad Automotor, Dirección de Rentas, Dependencias Policiales, Oficinas Municipales y Compañias de Seguros.',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671737283/app-roldan-asociadas/OIP_rhwamx.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
    [
      'Jubilaciones y Pensiones',
      'Consultoria Previsional, Jubilación Planificada, Pensión por Fallecimiento, Jubilación para Autónomos.',
      'https://res.cloudinary.com/dse7tzeho/image/upload/v1671739528/app-roldan-asociadas/pensiones_sj0mji.jpg',
      [
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
        ['Pregunta Frecuente', 'Respuesta Frecuente'],
      ],
    ],
  ],
}
